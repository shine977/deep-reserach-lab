...
export class WorkflowStreamService {
  ...
  private createNodePipelines(
    ...
    const pipelines = new Map<
      string,
      (item: StreamItem) => Observable<StreamItem>
    >();
    ...
    workflow.nodes.forEach((node) => {
      ...
      const pipeline = (inputItem: StreamItem): Observable<StreamItem> => {
        ...
        const context: NodeContext = {
          ...
          logger: console,
          emitEvent: (type, data) =>
            this.emitNodeEvent(type, executionId, node.id, data),
        };

        // Emit node start event
        this.emitNodeEvent("node:start", executionId, node.id, {
          input: inputItem.data,
          config: node.config,
        });
        ...
        return processor(inputItem.data).pipe(
          ...
          catchError((error) => {
            this.emitNodeEvent("node:error", executionId, node.id, {
              error,
              input: inputItem.data,
            });
          ...
          tap((outputItem) => {
            this.emitNodeEvent("node:complete", executionId, node.id, {
              input: inputItem.data,
              output: outputItem.data,
            });
          }),
