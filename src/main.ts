...
function createDefaultWorkflow(): Workflow {
  return {
    ...
    description:
      "A workflow that searches, reads web content, reasons about it, and manages token budget",
    nodes: [
      {
        id: "search-node",
        type: "search",
...
async function main() {
  ...
  try {
    ...
    if (!query) {
      console.error(
        "No query provided. Please provide a query as a command line argument.",
      );
      console.log(
        'Example: npm run dev "What are the latest developments in AI?"',
      );
      process.exit(1);
    }
      ...
      next: (result) => {
        ...
        if (result.status === "completed") {
          ...
          if (result.result && result.result.budget) {
            ...
            logger.info(
              `Token Usage: ${budget.used}/${budget.total} tokens used (${budget.remaining} remaining)`,
            );
          }
        } else {
