...
import { Plugin, NodePlugin, ValidationResult } from '../../core/types/plugin.types';
...
export class PluginRegistry {
  ...
  registerPlugin(plugin: Plugin): ValidationResult {
    ...
    if (this.isNodePlugin(plugin)) {
      this.nodePlugins.set((plugin as NodePlugin).nodeType, plugin as NodePlugin);
    }
  ...
  private validatePlugin(plugin: Plugin): ValidationResult {
    ...
    if (this.isNodePlugin(plugin)) {
      ...
      if (nodePlugin.nodeType && this.nodePlugins.has(nodePlugin.nodeType)) {
        errors.push(`Node plugin with type "${nodePlugin.nodeType}" is already registered`);
      }
    }
