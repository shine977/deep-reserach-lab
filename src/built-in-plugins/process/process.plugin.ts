...
export class ProcessPlugin implements NodePlugin {
  ...
  async initialize(context: PluginContext): Promise<void> {
    context.logger.info("Initializing process plugin");
  }
  ...
  process(input: any, config: any, context: ExecutionContext): Observable<any> {
    context.logger.info(`Processing data in process node`);
    ...
    try {
      ...
      if (config?.customFunction) {
        try {
          ...
          context.logger.error(`Error in custom function: ${error.message}`);
        }
      }

      return of({ output: result });
    } catch (error: any) {
      context.logger.error(`Error processing data: ${error.message}`);
