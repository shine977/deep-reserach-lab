...
export class TokenBudgetPlugin implements NodePlugin {
  ...
  async initialize(context: PluginContext): Promise<void> {
    context.logger.info('Initializing token budget plugin');
  }
  ...
  process(input: any, config: any, context: ExecutionContext): Observable<any> {
    ...
    return new Observable(observer => {
      ...
      } else if (this.budget.remaining < this.budget.total * (config.warningThreshold || 0.8)) {
        context.logger.warn(`Token budget running low: ${this.budget.remaining} tokens remaining`);
      }
  ...
  private updateBudget(tokensUsed: number, source: string): void {
    ...
    this.budget.details[source] = (this.budget.details[source] || 0) + tokensUsed;
  }
