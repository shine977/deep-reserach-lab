...
export class SearchPlugin implements NodePlugin {
  ...
  private mockSearch(query: string, limit: number, context: ExecutionContext): Observable<any> {
  ...
  private searchWithJina(query: string, limit: number, context: ExecutionContext): Observable<any> {
    return new Observable(observer => {
      // Check for API key
      ...
      // Make API request
      axios.get('https://api.jina.ai/search', {
        params: {
          query,
          limit
        },
