...
export class WebReaderPlugin implements NodePlugin {
  ...
  private mockFetchContent(url: string, maxLength: number, context: ExecutionContext): Observable<ContentItem> {
    return new Observable(observer => {
      setTimeout(() => {
        observer.next({
          url,
          title: `Content from ${url}`,
          content: `This is mock content for ${url}. It simulates fetched web content for testing purposes.
...
The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog.`.slice(0, maxLength)
        });
        observer.complete();
  ...
  private fetchContent(url: string, maxLength: number, context: ExecutionContext): Observable<ContentItem> {
    return new Observable(observer => {
      axios.get(url)
        .then(response => {
          // Extract title
