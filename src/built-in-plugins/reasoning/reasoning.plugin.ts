...
export class ReasoningPlugin implements NodePlugin {
  ...
  process(input: any, config: any, context: ExecutionContext): Observable<any> {
    ...
    context.logger.info(`Processing reasoning with ${contents.length} content items`);
  ...
  private buildPrompt(query: string, contents: any[], additionalContext: string): string {
    ...
    prompt += '3. Confidence: A number between 0 and 1 indicating your confidence\n';
    prompt += '4. Next Action: What to do next (search, visit, answer, or reflect)\n';
    
    return prompt;
  }
  
  // Mock reasoning for testing
  private mockReasoning(prompt: string, context: ExecutionContext): Observable<ReasoningResult> {
...
Step 3: Synthesized the information to formulate a comprehensive answer.`,
          ...
          confidence: 0.85,
          nextAction: ['search', 'visit', 'answer', 'reflect'][Math.floor(Math.random() * 4)]
        };
  ...
  // Reason with Gemini
  private reasonWithGemini(prompt: string, temperature: number, context: ExecutionContext): Observable<ReasoningResult> {
    return new Observable(observer => {
      // Check for API key
      ...
      // Make API request
      axios.post(`${apiUrl}?key=${geminiApiKey}`, payload)
        .then(response => {
          try {
            ...
            const confidenceMatch = /Confidence:(.*?)(?:Next Action:|$)/s.exec(text);
            const nextActionMatch = /Next Action:(.*?)(?:$)/s.exec(text);
            
            const result: ReasoningResult = {
              reasoning: reasoningMatch ? reasoningMatch[1].trim() : 'No reasoning provided',
              answer: answerMatch ? answerMatch[1].trim() : 'No answer provided',
              confidence: confidenceMatch ? parseFloat(confidenceMatch[1].trim()) || 0.5 : 0.5,
              nextAction: nextActionMatch ? nextActionMatch[1].trim().toLowerCase() : 'reflect'
            };
  ...
  // Reason with OpenAI
  private reasonWithOpenAI(prompt: string, temperature: number, context: ExecutionContext): Observable<ReasoningResult> {
    return new Observable(observer => {
      // Check for API key
