...
import { ExecutionRecord, ExecutionFilter, ExecutionStatus, ExecutionBranch, BranchStatus, BranchFilter } from '../models/execution.types';
...
export class ExecutionStorageService {
  ...
  async saveBranch(branch: ExecutionBranch): Promise<void> {
    ...
    console.log(`Saved branch record: ${branch.id} for execution: ${branch.executionId}`);
  }
  ...
  async getExecutionBranches(executionId: string): Promise<ExecutionBranch[]> {
    return Array.from(this.branchRecords.values())
  ...
  async listBranches(filter: BranchFilter = {}): Promise<ExecutionBranch[]> {
    ...
    if (filter.parentBranchId) {
      branches = branches.filter(b => b.parentBranchId === filter.parentBranchId);
    }
