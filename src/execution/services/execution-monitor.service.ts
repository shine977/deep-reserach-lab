...
export class ExecutionMonitorService {
      ...
      next: (data) => {
        ...
        if (data.type === 'node:complete') {
          ...
          if (nodeMetrics) {
            ...
            nodeMetrics.duration = nodeMetrics.endTime.getTime() - nodeMetrics.startTime.getTime();
        ...
        if (data.type === 'node:error') {
          ...
          if (data.branchId) {
            this.trackBranchNodeError(data.branchId, data.nodeId, data.data?.error);
          }
        }
  ...
  async collectExecutionMetrics(executionId: string): Promise<ExecutionMetrics> {
    ...
    Object.values(metrics.nodeMetrics).forEach(nodeMetric => {
      if (nodeMetric.startTime && (!earliestStartTime || nodeMetric.startTime < earliestStartTime)) {
    ...
    for (const [branchId, branchMetric] of this.branchMetrics.entries()) {
      if (branchMetric.executionId === executionId) {
        ...
        const completed = nodeIds.filter(id => branchMetric.nodeMetrics[id].endTime).length;
  ...
  getExecutionBranchProgress(executionId: string): BranchProgress[] {
    return Array.from(this.branchProgress.values())
      .filter(progress => progress.executionId === executionId);
  }
  
  /**
   * Track when a branch starts
   */
  private trackBranchStart(branchId: string, executionId: string, data: any): void {
   ...
   */
  private trackBranchNodeStart(branchId: string, nodeId: string, executionId: string): void {
   ...
   */
  private trackBranchNodeComplete(branchId: string, nodeId: string, data: any): void {
    const branchMetric = this.branchMetrics.get(branchId);
    const progress = this.branchProgress.get(branchId);
    ...
    if (progress) {
      ...
      progress.pendingNodeIds = progress.pendingNodeIds.filter(id => id !== nodeId);
      
      // Update progress percentage
      const totalNodes = progress.completedNodeIds.length + progress.pendingNodeIds.length;
      progress.progress = Math.round((progress.completedNodeIds.length / totalNodes) * 100);
   ...
   */
  private trackBranchNodeError(branchId: string, nodeId: string, error: any): void {
    const progress = this.branchProgress.get(branchId);
