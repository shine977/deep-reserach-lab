...
export class SearchPlugin implements NodePlugin {
  ...
  async initialize(context: PluginContext): Promise<void> {
    this.logger.info("Initializing search plugin");
  }

  async activate(): Promise<void> {
    this.logger.log("Search plugin activated");
  }

  async deactivate(): Promise<void> {
    this.logger.log("Search plugin deactivated");
  }

  // Process method
  process(input: any, config: any, context: ExecutionContext): Observable<any> {
    this.logger.info(`Processing search query: ${input.query}`);
  ...
  private searchWithJina(
    ...
    return new Observable((observer) => {
      ...
      axios
        ...
        .catch((error) => {
          this.logger.error("Jina search error:", error);
